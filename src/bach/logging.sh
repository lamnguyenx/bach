...
function log() { (
    separate_heading "$@"
    printf "%s" "$(log_date)$(echo_bold_cyan $p_level_tag)$1"
    shift
    echo " $*"
) >&2; }
function log_bold() { (
    separate_heading "$@"
    printf "%s" "$(log_date)$(echo_bold_cyan $p_level_tag)$(echo_bold "$1")"
    shift
    echo " $*"
) >&2; }
function log_red() { (
    separate_heading "$@"
    printf "%s" "$(log_date)$(echo_bold_cyan $p_level_tag)$(echo_bold_red "$1")"
    shift
    echo " $*"
) >&2; }
function log_green() { (
    separate_heading "$@"
    printf "%s" "$(log_date)$(echo_bold_cyan $p_level_tag)$(echo_bold_green "$1")"
    shift
    echo " $*"
) >&2; }
function log_yellow() { (
    separate_heading "$@"
    printf "%s" "$(log_date)$(echo_bold_cyan $p_level_tag)$(echo_bold_yellow "$1")"
    shift
    echo " $*"
) >&2; }
function log_gray() { (
    separate_heading "$@"
    printf "%s" "$(log_date)$(echo_bold_cyan $p_level_tag)$(echo_bold_gray "$1")"
    shift
    echo " $*"
) >&2; }
function log_cyan() { (
    separate_heading "$@"
    printf "%s" "$(log_date)$(echo_bold_cyan $p_level_tag)$(echo_bold_cyan "$1")"
    shift
    echo " $*"
) >&2; }
